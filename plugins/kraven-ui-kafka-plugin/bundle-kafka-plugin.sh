...
# Script to build and bundle the Kafka plugin for Maven Central deployment
...
echo "Building Kafka plugin bundle version: $VERSION"
...
BUNDLE_DIR="dist/plugins/kafka"
MAVEN_BUNDLE_DIR="dist/maven-bundles"
mkdir -p $BUNDLE_DIR
mkdir -p $MAVEN_BUNDLE_DIR

# Copy plugin JAR to bundle directory
echo "Copying Kafka plugin JAR to bundle directory..."
cp plugins/kraven-ui-kafka-plugin/target/kraven-ui-kafka-plugin-*-jar-with-dependencies.jar $BUNDLE_DIR/
...
ARTIFACT_BASE="kraven-ui-kafka-plugin"
...
cp plugins/kraven-ui-kafka-plugin/target/kraven-ui-kafka-plugin-*-jar-with-dependencies.jar \
   $MAVEN_BUNDLE_TMP/$GROUP_PATH/$ARTIFACT_ID/$VERSION/$ARTIFACT_ID-$VERSION.jar

# Copy POM file
cp plugins/kraven-ui-kafka-plugin/pom.xml \
...
echo "Kafka plugin bundle created successfully:"
