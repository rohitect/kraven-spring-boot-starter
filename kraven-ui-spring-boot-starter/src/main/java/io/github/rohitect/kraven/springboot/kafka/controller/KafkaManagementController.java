 ...
 */
@RestController
@RequestMapping("/kraven/api/kafka-management")
...
public class KafkaManagementController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved cluster info", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaClusterInfo.class))),
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved brokers", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaBroker.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved topics", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaTopic.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved consumer groups", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaConsumerGroup.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved listeners", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaListener.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved topic", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaTopic.class))),
        ...
        public ResponseEntity<KafkaTopic> getTopicByName(@PathVariable("name") String name) {
                log.debug("Getting topic by name: {}", name);
                ...
                log.debug("Found {} topics in cluster info", clusterInfo.getTopics().size());
                clusterInfo.getTopics().forEach(topic -> log.debug("Available topic: {}", topic.getName()));

                return clusterInfo.getTopics().stream()
                                .filter(topic -> {
                                        boolean matches = topic.getName().equals(name);
                                        log.debug("Topic {} matches requested name {}: {}", topic.getName(), name,
                                                        matches);
                                        return matches;
                                })
                                .findFirst()
        ...
        public ResponseEntity<List<KafkaConsumerGroup>> getConsumersForTopic(@PathVariable("name") String name) {
                ...
                log.debug("Found {} consumer groups in cluster info", clusterInfo.getConsumerGroups().size());

                List<KafkaConsumerGroup> consumersForTopic = clusterInfo.getConsumerGroups().stream()
                                .filter(group -> {
                                        if (group.getTopicPartitions() == null) {
                                                log.warn("Consumer group {} has no topic partitions",
                                                                group.getGroupId());
                                                return false;
                                        }
                                        ...
                                        boolean matches = group.getTopicPartitions().stream()
                                                        .anyMatch(tp -> {
                                                                ...
                                                                log.debug("Topic partition {} matches requested topic {}: {}",
                                                                                tp.getTopic(), name, topicMatches);
                                                                return topicMatches;
                                                        });

                                        log.debug("Consumer group {} matches topic {}: {}", group.getGroupId(), name,
                                                        matches);
                                        return matches;
                                })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved messages", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaMessage.class))),
        ...
        public ResponseEntity<Map<String, Object>> getMessagesFromTopic(
                        ...
                        @Parameter(description = "Maximum number of messages per page") @RequestParam(name = "limit", defaultValue = "0") int limit,
                        @Parameter(description = "Page number (0-based)") @RequestParam(name = "page", defaultValue = "0") int page,
                        @Parameter(description = "Sort order: 'new' for newest first, 'old' for oldest first") @RequestParam(name = "sort", defaultValue = "new") String sort) {
                ...
                log.debug("Getting messages from topic: {}, limit: {}, page: {}, sort: {}", name, actualLimit, page,
                                sort);

                boolean sortNewestFirst = !"old".equalsIgnoreCase(sort);
                int offset = page * actualLimit;
                List<KafkaMessage> messages = kafkaMessageService.getMessagesFromTopic(name, actualLimit, offset,
                                sortNewestFirst);
        ...
        @PostMapping("/topics/{name}/messages")
        @Operation(summary = "Send a message to a topic", description = "Sends a message to a specific Kafka topic")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully sent message", content = @Content(mediaType = "application/json", schema = @Schema(implementation = KafkaMessage.class))),
                        @ApiResponse(responseCode = "403", description = "Message production is disabled", content = @Content),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
        })
        public ResponseEntity<KafkaMessage> sendMessageToTopic(
                ...
                if (!kafkaAdminService.isMessageProductionEnabled()) {
                        ...
                        throw new org.springframework.web.server.ResponseStatusException(
                                        org.springframework.http.HttpStatus.FORBIDDEN,
                                        "Message production is disabled in the configuration. To enable it, set 'kraven.ui.kafka.message-production-enabled=true' in your application properties.");
                }
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully established stream connection", content = @Content(mediaType = "text/event-stream")),
        ...
        public SseEmitter streamMessagesFromTopic(
                ...
                emitter.onCompletion(() -> {
                        log.debug("SSE stream completed for topic: {}", name);
                        emitters.remove(emitter);
                });

                emitter.onTimeout(() -> {
                        log.debug("SSE stream timed out for topic: {} after {} ms", name, sseTimeoutMs);
                        emitters.remove(emitter);
                });
